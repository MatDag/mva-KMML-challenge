...
def kernel_full(matrix_1, matrix_2, parameters):
    ...
    K += alpha_2**2 *np.exp(-np.sqrt(matrix_norm)/(sigma_2**2))
    
    # Polynomial
    #alpha_3 = parameters[4]
    #d  = parameters[5]
    #K+= alpha_3 * matrix_inner**(d**2 + 1)
    
    # Rational quadratic
    alpha_4 = parameters[4]
    beta_RQ = parameters[5]
    alpha_RQ = parameters[6]
    K += alpha_4**2 *(beta_RQ**2 + matrix_norm)**(-(alpha_RQ**2 + epsilon))
    
    # Cauchy
    alpha_5 = parameters[7]
    sigma_cauchy = parameters[8]
...
kernels_dic = {"RBF" : kernel_RBF,"poly": kernel_poly, "Laplacian": kernel_laplacian, 
               ...
               "Periodic": kernel_periodic, "full" : kernel_full
               }
